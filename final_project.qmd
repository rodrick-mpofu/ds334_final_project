...
2.  What are the fighting stances in the UFC?
3.  What are the most common ways to win a fight?
4.  What are the most common ways to lose a fight?
5.  What are the most common ways to win a fight by decision?
6.  What are the most common ways to win a fight by knockout?
7.  What are the most common ways to win a fight by submission?
8.  What are the most common ways to win a fight by doctor stoppage?
9.  What are the most common ways to win a fight by split decision?

And the shiny app will provide the following functionalities:

1. Show the trends of different fighter statistics over time
2. Allow users to compare two fighters and see their statistics side by side
...
ggplot(data_df, aes(x = year(date))) +
  geom_bar(fill = "purple2", color = "black") +
  theme_minimal() +
...
```{r}
# Use same idea as previous example
# Compare the stats of two fighters
...
radar_data <- rbind(max_values, min_values, fighter_comp[1:6])

#radar_data <- radar_data |> mutate(fighter = c("1", "2", fighter_comp$fighter))
...
radarchart(radar_data, axistype=0, 
           ...
           #custom the grid
           cglcol="grey", cglty=1, 
           axislabcol="black", cglwd=0.8,
           #custom labels
           vlcex=0.6)

# Add a legend
# Make sure the colors are the same as the radar chart and match the order
legend(x= -1, 
       y=-1.2,  
       legend = c("Conor McGregor", "Khabib Nurmagomedov"), 
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    # plot with default options:
    radarchart(fighter_full_df(), axistype=0, 
               maxmin=T,
               #custom polygon
               pcol=colors_border , 
               pfcol=colors_in , 
